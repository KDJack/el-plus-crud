  ...
  <template v-if="vif">
    ...
    <template v-else>
      <template v-if="props.desc?.confirm">
        <el-popconfirm @confirm="onEvents.click" :title="props.desc?.confirm">
          <template #reference>
            <el-button :loading="localLoading" :size="props.desc?.size || defaultConf.size" v-bind="attrs" :class="{ btnHideLabel: !props.desc?.label && !$slots.default }">
              <slot name="default">
                <template v-if="!!props.desc?.label">{{ btnShowText }}</template>
              </slot>
            </el-button>
      ...
      <el-button v-else :loading="localLoading" :size="props.desc?.size || defaultConf.size" v-bind="attrs" v-on="onEvents" :style="{ pointerEvents: desc?.isTag ? 'none' : 'all' }" :class="{ 'no-label': !props.desc?.label, btnHideLabel: !props.desc?.label && !$slots.default }">
        <slot name="default">
          <template v-if="!!props.desc?.label">{{ btnShowText }}</template>
        </slot>
      </el-button>
